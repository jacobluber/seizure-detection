...
class CustomVAE(VAE):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.decoder.upscale1=Indentity()
        #self.decoder.upscale1=Interpolate(16)
            
        
        """
        ##############encoder#############
        
        x=torch.zeros(256,3,136,152)
        ...
        self.decoder.linear = nn.Linear(latent_dim, 512 * 17 * 19)
        #test=torch.zeros(256,32)
        x= self.decoder.linear(z)
        print(x.shape)

        # NOTE: replaced this by Linear(in_channels, 514 * 4 * 4)
        # x = F.interpolate(x, scale_factor=4)

        x= x.view(x.size(0), 512 * self.decoder.expansion, 17, 19)
        print(x.shape)
        x= self.decoder.upscale1(x)
        print(x.shape)
    ...
    def training_step(self, batch, batch_idx):
        ...
        latent_spaces_dir = join(f"{self.logger.save_dir}/{self.logger.name}/version_{self.logger.version}", "train_latent_spaces")
        recons_images_dir = join(f"{self.logger.save_dir}/{self.logger.name}/version_{self.logger.version}", "train_recons_images")

        try:
            os.makedirs(latent_spaces_dir)
            os.makedirs(recons_images_dir)

        except FileExistsError:
    # Directory already exists, handle accordingly (e.g., rename, create with a different name, etc.)
            pass


        #create_dir(predict_dir)
        #create_dir(recons_images_dir)
        ...
        for i, file in enumerate(files):
            name = basename(file.split('.npy')[0])
            a=z[i].cpu().detach()
            
            #save_latent_space(z[i], join(latent_spaces_dir, f"{name}.data"))
            torch.save(a, join(latent_spaces_dir, f"{name}.pt"))
    ...
    def validation_step(self, batch, batch_idx):
        ...
        #loss, logs = self.step(batch, batch_idx)
        #print(f"{self.logger.save_dir}/{self.logger.name}/version_{self.logger.version}")
     

        #predict_dir = join(f"{self.logger.save_dir}/{self.logger.name}/version_{self.logger.version}", "test_patches")
        latent_spaces_dir = join(f"{self.logger.save_dir}/{self.logger.name}/version_{self.logger.version}", "val_latent_spaces")
        recons_images_dir = join(f"{self.logger.save_dir}/{self.logger.name}/version_{self.logger.version}", "val_recons_images")
    ...
    def test_step(self, batch, batch_idx):
        ...
        latent_spaces_dir = join(f"{self.logger.save_dir}/{self.logger.name}/version_{self.logger.version}", "test_latent_spaces")
        recons_images_dir = join(f"{self.logger.save_dir}/{self.logger.name}/version_{self.logger.version}", "test_recons_images")
    ...
    def test_epoch_end(self, output):
        if self.global_rank == 0:
            ...
            for i in range(len(output)):
                 test_loss=output[i]+test_loss

            test_loss=test_loss/len(output)
